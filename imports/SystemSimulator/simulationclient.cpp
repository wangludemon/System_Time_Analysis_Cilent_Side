    ...
    : QObject{parent}
{
    ...
    qDebug() << "SimulationClient initialized, baseUrl =" << m_baseUrl;
}

// === 新增：设置服务器 IP ===
void SimulationClient::setServerIp(const QString &ip)
{
    if (m_serverIp == ip)
        return;
    ...
    qDebug() << "SimulationClient server IP updated:" << m_serverIp
             << ", baseUrl =" << m_baseUrl;
...
// === 新增：设置服务器端口 ===
void SimulationClient::setServerPort(int port)
{
    ...
    qDebug() << "SimulationClient server port updated:" << m_serverPort
             << ", baseUrl =" << m_baseUrl;
...
void SimulationClient::sendSimulationRequest()
{
    ...
    // QML 中是 "Short Length"，后端可能要求 "SHORT LENGTH"
    ...
    qDebug() << "Sending Simulation Request to:" << request.url().toString();
    qDebug() << "Config:"
...
void SimulationClient::getProtocolData(QString protocolName)
{
    ...
    qDebug() << "Requesting protocol data from:" << request.url().toString();
...
void SimulationClient::onReplyFinished(QNetworkReply *reply)
{
    ...
    if (rootObj.contains("cpuGanttInformations")) {
        ...
        for (const auto &val : rawCpuArray) {
            ...
            if (switchTime > 0 && cpuObj.contains("eventTimePoints")) {
    ...
    if (rootObj.contains("taskGanttInformations")) {
        ...
        for (const auto &val : rawArray) {
            sortedList.append(val.toObject());
        }
